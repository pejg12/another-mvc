...
class CMModules extends CObject {
   ...
   * Properties
   */
  private $amvcCoreModules = array('CAmvc', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $amvcCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
  ...
  private function GetDetailsOfModule($module) {
    ...
    if(class_exists($module)) {
      ...
      $details['isAmvcCore']        = in_array($rc->name, $this->amvcCoreModules);
      $details['isAmvcCMF']         = in_array($rc->name, $this->amvcCMFModules);
